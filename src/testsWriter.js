...
const generatePropAttributes =
  require('./propsGenerator').generatePropAttributes
...
const testsFileTemplate = `import React from 'react';
...
const testTemplate = `test('$COMPONENT_NAME $TEST_ID', () => {
...
function generateTest(componentName, id, propsText) {
  ...
  testContent =
    testContent
      ...
      .replace(/\$COMPONENT_PROPS/g, propsText) + '\n'
...
function generateComponentImportStatement(componentName, enums) {
  ...
  if (enums) {
    result += ', { ' + enums.map((item) => item.name).join(', ') + ' }'
  }
...
function generateTestsFile(componentPath, componentName, enums, props) {
  const tests = getTestContents(componentName, props, enums).join('')

  let fileContent = testsFileTemplate.replace(
    /\$COMPONENT_IMPORT/g,
...
function writeTestsFile(componentName, componentPath, props, enums, testsRoot) {
  ...
  const testsFileName = `${testPath}/${getComponentNameFromPath(
    componentPath
  )}.js`
