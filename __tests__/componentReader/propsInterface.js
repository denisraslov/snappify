...
test('Props interface parsing', () => {
    expect(findPropsInterface(
      prepareTextForParsing(propsIntefaceText1))
    ).toEqual(parsedProps);

    expect(findPropsInterface(
      prepareTextForParsing(propsIntefaceText2))
    ).toEqual(parsedProps);
});
